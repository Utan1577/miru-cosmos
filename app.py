...
# NM: result is last2 digits of N3, payout will be fixed later (Rakuten mini row)
...
# ------------------------------------------------------------
# UI (NOW: result hide + preds centered, BACK: legend + payout align)
# ------------------------------------------------------------
html_code = f"""
<!DOCTYPE html>
<html>
<head>
  ...
  <style>
    ...
    .pay-row {{ display:flex; justify-content:space-between; font-size:11px; font-weight:800; line-height:1.15; white-space:nowrap; }}
    .pay-label {{ min-width:34px; }}
    .pay-amt {{ min-width:92px; text-align:right; }}

    .legend {{ position:absolute; right:10px; bottom:8px; font-size:9px; color:#333; opacity:0.9; }}
    .legend .bx {{ color:#ff3b30; }}
    .legend .str {{ color:#007aff; }}
    ...
    .lcd.mode-now .legend {{ display:none; }}
...
<body>
  ...
  <script>
    ...
    function renderResultPanel(page){{
      ...
      let h="";
      // Only two lines: 第xxxx回 / YYYY/MM/DD
      h+=`<div class="result-line">第${{escHtml(rno)}}回</div>`;
      if(dt) h+=`<div class="result-line">${{escHtml(dt)}}</div>`;
      h+=`<div class="result-spacer"></div>`;
      h+=`<div class="result-line">当選: <span class="result-win">${{escHtml(res)}}</span></div>`;
      h+=`<div class="result-spacer"></div>`;

      if(curG==='NM'){{
        const miniY=payoutYen(pay,"MINI");
        h+=`<div class="pay-row"><span class="pay-label">Mini</span><span class="pay-amt">${{escHtml(miniY)}}</span></div>`;
        return h;
      }}
      ...
      if(strY) h+=`<div class="pay-row"><span class="pay-label">STR</span><span class="pay-amt">${{escHtml(strY)}}</span></div>`;
      if(boxY) h+=`<div class="pay-row"><span class="pay-label">BOX</span><span class="pay-amt">${{escHtml(boxY)}}</span></div>`;
      if(ssY)  h+=`<div class="pay-row"><span class="pay-label">S-S</span><span class="pay-amt">${{escHtml(ssY)}}</span></div>`;
      if(sbY)  h+=`<div class="pay-row"><span class="pay-label">S-B</span><span class="pay-amt">${{escHtml(sbY)}}</span></div>`;
      return h;
    }}

    // N4/N3: STR(position)=BLUE, BOX(multiset)=RED
    function renderMarkedDigitsSB(pred,result){{
      ...
      const isBlue=Array(pr.length).fill(false); // STR
      const isRed=Array(pr.length).fill(false);  // BOX

      for(let i=0;i<Math.min(pr.length,res.length);i++) {{
        if(pr[i]===res[i] && counts[pr[i]]>0) {{
          isBlue[i]=true; counts[pr[i]]--;
        }}
      }}
      for(let i=0;i<pr.length;i++) {{
        if(isBlue[i]) continue;
        const ch=pr[i];
        if(counts[ch] && counts[ch]>0) {{
          isRed[i]=true; counts[ch]--;
        }}
      }}
    ...
    // NM: STR only (position match). BOX ignored.
    function renderMarkedDigitsNM(pred,result){{
      const res=String(result||"");
      const pr=String(pred||"");
      let out="";
      for(let i=0;i<Math.min(pr.length,res.length);i++) {{
        const ch=pr[i];
        if(ch===res[i]) out+=`<span class="blue">${{escHtml(ch)}}</span>`;
        else out+=escHtml(ch);
      }}
      for(let i=res.length;i<pr.length;i++) out+=escHtml(pr[i]);
      return out;
    }}
    ...
    function renderPredPanel(page){{
      ...
      for(let i=0;i<Math.min(curC,preds.length);i++) {{
        const v=preds[i];
        if(page.mode==='RESULT' && res && (curG==='N4'||curG==='N3')) {{
          h+=`<div class="num-text">${{renderMarkedDigitsSB(v,res)}}</div>`;
        }} else if(page.mode==='RESULT' && res && curG==='NM') {{
          h+=`<div class="num-text">${{renderMarkedDigitsNM(v,res)}}</div>`;
        }} else {{
    ...
    function update(){{
      ...
      if(page.mode==='RESULT' && (curG==='N4' || curG==='N3')) {{
        legend.style.display='block';
        legend.innerHTML='<span class="bx">🟥BX</span> <span class="str">🟦STR</span>';
      }} else {{

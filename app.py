...
# =========================
# MIRU-PAD (UI fixed / Core fixed + FIXED URL + meta 표시)
# =========================
...
def default_status():
    return {
        ...
        "N4_LATEST_R": "----",
        "N3_LATEST_R": "---",
        "N4_LATEST_INFO": "NO DATA",
        "N3_LATEST_INFO": "NO DATA",
        "LATEST_AT": ""
    }
...
def _t(cell):
    return cell.get_text(" ", strip=True).replace("\u3000", " ").replace("\xa0", " ")

def _find_value_in_same_table(table, key_text):
    for cell in table.find_all(["th", "td"]):
        if key_text in _t(cell):
...
def _parse_blocks_numbers(soup, digits, max_rows):
    ...
    for cell in soup.find_all(["th", "td"]):
        txt = _t(cell)
        if ("抽せん数字" not in txt) and ("抽選数字" not in txt):
            continue
...
def fetch_history(game_type, max_rows=20):
    ...
    try:
        ...
        info_str = f'{info.get("round","")} {info.get("date","")}'.strip()
        if not info_str:
            info_str = "INFO N/A"

    except Exception as e:
        # ★ダミー数字で誤魔化さない。取れてないのを明示する
        history = [[0]*digits, [0]*digits]
        last_val = "----" if digits == 4 else "---"
        info_str = f"FETCH FAIL"

    trends = {}
    for i, col in enumerate(cols):
        spins = []
        for j in range(len(history) - 1):
            curr_idx = INDEX_MAP[col].get(history[j][i], 0)
            prev_idx = INDEX_MAP[col].get(history[j+1][i], 0)
...
def generate_predictions(game_type, last_val, trends):
    ...
    # last_val が取れてない時は固定のダミーを出す（勝負には使わない想定）
    if not last_val or last_val in ("----", "---"):
        return ["----"]*10 if game_type == "N4" else ["---"]*10
    ...
    for role in roles:
        ...
        for attempt in range(30):
            ...
            for i, col in enumerate(cols):
                ...
                jitter = random.choice([0, 1, -1, 2, -2, 5]) if attempt > 0 else 0
...
def generate_unique_mini(n3_preds, n3_last_val, n3_trends):
    if not n3_last_val or n3_last_val in ("---",):
        return ["--"]*10
    mini_preds = []
    seen_mini = set()
    cols = ['n2', 'n3']
    ...
    for i, n3_val in enumerate(n3_preds):
        candidate = n3_val[-2:] if n3_val and n3_val not in ("---",) else "--"
        role = roles[i]
...
# 20:00以降は latest だけ更新（液晶は古い世代のまま）
if is_after_20():
...
# 日次初回だけ作る（コロコロ変わらない）
need_refresh = (status.get("date_key") != target_key) or (status.get("N4_R", "----") in ("----", "", None))
if need_refresh:
    n4_l, n4_info, n4_t, _ = fetch_history('N4', 20)
    n3_l, n3_info, n3_t, _ = fetch_history('N3', 20)
...
# LAST RESULT 表示は 20:00以降 latest

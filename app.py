...
def fetch_kc_results_mizuho(need: int = 20):
    ...
    try:
        ...
        items = []
        # 最新のテーブルだけ見ればOK（月別ではないトップページ想定）
        for tr in tables[0].select("tr"):
            th = tr.select_one("th.bgf7f7f7") # 回号・日付
            tds = tr.select("td")
            ...
            if len(fruits) != 4:
                continue # 絵柄が取れなければスキップ
                
            result_str = "".join(fruits)
            
            # 払戻金 (tds[1]が当せん金額)
            # 構造: 1等 xxxx円 2等 xxxx円 ... とテキストが続くか、改行されている
            # みずほのHTML構造に合わせて解析
            payout_text = tds[1].get_text(" ", strip=True)
            
            payout = {}
            # 正規表現で金額抽出 (例: 1等 4,500円)
            m_p1 = re.search(r"1等\s*([\d,]+)円", payout_text)
            m_p2 = re.search(r"2等\s*([\d,]+)円", payout_text)
            m_p3 = re.search(r"3等\s*([\d,]+)円", payout_text)
...
def ensure_predictions_for_round(game: str, round_no: int, base_last: str, base_trends: dict, base_pred_func) -> list[str]:
    ...
    # KCの場合は base_pred_func が引数なし(または無視)で動く想定
    if game == "KC":
        preds = base_pred_func() # kc_random_10()
    else:
...
def build_pages_for_game(game: str, items: list, months_used: list[int]) -> dict:
    ...
    for it in norm_items:
        ...
        # KCは履歴からTrend計算しない(毎回ランダムシード)ためダミー
...
<head>
  ...
  <style>
    ...
    /* KCの場合はフルーツ判定で色を乗せるが、Emoji自体に色があるのでborderやbgで表現も手だが、
       要望通り「赤と青」にするため、text-shadow等で工夫するか、単純に機能させる */
...
<body>
  ...
  <script>
    ...
    function renderMarkedDigitsSB(pred,result){{
      ...
      for(let i=0;i<prArr.length;i++) {{
        const ch=prArr[i];
        // KCの場合はEmojiなのでspanで囲む際、視認性のためにShadow等を使うか、そのままClass付与

...
from fetch import fetch_last_n_results

# =========================
# MIRU-PAD (RAKUTEN/TS4 SOURCE / AUTO / BACK-NEXT-NOW)
# (2-file standalone: app.py + fetch.py)
# =========================
...
# Config (standalone)
# ------------------------------------------------------------
JST = timezone.utc  # placeholder (avoid timezone dep); display is not critical here
STATUS_FILE = "miru_status.json"

# ------------------------------------------------------------
# Model / Prediction helpers (standalone)
# ------------------------------------------------------------
def calc_trends_from_history(history_nums: list[list[int]], cols: list[str]) -> dict:
    """
    history_nums: [[n1,n2,n3,n4], ...] (desc or asc ok)
    returns simple frequency trends for weighted prediction
    """
    if not history_nums:
        return {"total": 0, "pos_freq": [], "all_freq": {}}

    digits = "0123456789"
    length = len(history_nums[0])
    pos_freq = [{d: 1 for d in digits} for _ in range(length)]  # +1 smoothing
    all_freq = {d: 1 for d in digits}  # +1 smoothing

    for row in history_nums:
        for i, n in enumerate(row):
            d = str(int(n))
            pos_freq[i][d] = pos_freq[i].get(d, 1) + 1
            all_freq[d] = all_freq.get(d, 1) + 1

    return {
        "total": len(history_nums),
        "pos_freq": pos_freq,
        "all_freq": all_freq,
        "len": length,
    }
...
def _weighted_choice(freq_map: dict[str, int], rng: random.Random) -> str:
...
def generate_predictions(game: str, last_val: str, trends: dict) -> list[str]:
    """
    Generates 10 predictions as strings.
    Uses simple per-position digit frequency weighting + deterministic seed.
    """
    length = 4 if game == "N4" else 3
    seed_base = f"{game}|{last_val}|{trends.get('total',0)}"
    rng = random.Random(seed_base)

    pos_freq = trends.get("pos_freq") or [{str(d): 1 for d in range(10)} for _ in range(length)]
    # Ensure correct length
    if len(pos_freq) != length:
        pos_freq = [{str(d): 1 for d in range(10)} for _ in range(length)]

    out = []
    seen = set()
    guard = 0
    while len(out) < 10 and guard < 5000:
        guard += 1
        s = "".join(_weighted_choice(pos_freq[i], rng) for i in range(length))
        if s not in seen:
            seen.add(s)
            out.append(s)

    # fallback if somehow not enough unique
    while len(out) < 10:
        out.append("0" * length)
...
def kc_random_10() -> list[str]:
    rng = random.Random("KC|fixed")
    return [f"{rng.randrange(0, 10000):04d}" for _ in range(10)]
...
def save_status(s):
    # JSTがtimezone.utcでも、表示用updated_atなので実害なし
    s["updated_at"] = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
    try:
...
# ------------------------------------------------------------
def ensure_predictions_for_round(game: str, round_no: int, base_last: str, base_trends: dict, base_pred_func):
...
# ------------------------------------------------------------
# Fetch latest results, build NM from N3
# ------------------------------------------------------------

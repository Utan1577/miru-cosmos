...
def fetch_history(game_type):
    """
    指定されたゲーム(N4/N3)の履歴を取得し、直近の値と傾向を返す
    """
    if game_type == 'N4':
    ...
    try:
        ...
        for row in rows:
            ...
            if data:
                val = data.text.strip().replace(' ', '') # N3はスペースが入ることがあるので除去
                if val.isdigit() and len(val) == len(cols):
                    history.append([int(d) for d in val])
        
        if not history: raise Exception("No data")
            
    except:
        # 取得失敗時のダミー
        history = [[8,2,9,6], [1,2,3,4]] if game_type == 'N4' else [[3,5,8], [1,2,3]]
    ...
    for i, col in enumerate(cols):
        ...
        for j in range(len(history) - 1):
            curr = history[j][i]
            prev = history[j+1][i]
            c_idx = INDEX_MAP[col][curr]
            p_idx = INDEX_MAP[col][prev]
            spins.append((c_idx - p_idx) % 10)
        
        if spins:
            trends[col] = Counter(spins).most_common(1)[0][0]
        else:
            trends[col] = 0 # データ不足時は回転なしと仮定
...
def generate_best_mix(game_type, last_val, trends):
    """
    1口目：エース(本命)
    2口目：シフト(ズレ保険)
    3口目：カオス(大穴)
    ...
    # 役割分担リスト (10口分)
    # ace: ガチ理論, shift: ズレ狙い, chaos: 完全ランダム/大穴
    roles = ['ace', 'shift', 'chaos', 'ace', 'shift', 'ace', 'ace', 'shift', 'ace', 'chaos']
    ...
    for role in roles:
        ...
        for i, col in enumerate(cols):
            ...
            # スピン決定
            if role == 'chaos':
                spin = random.randint(0, 9) # 完全ランダム
            elif role == 'shift':
                # トレンドから少しズラす or 逆回転
                base = trends[col]
                shift_val = random.choice([1, -1, 5]) # 隣か裏
                spin = (base + shift_val) % 10
            else: # ace
                # トレンド通り (たまに揺らぎ)
                spin = trends[col] if random.random() > 0.2 else (trends[col] + random.randint(-1, 1)) % 10
            
            target_idx = (curr_idx + spin) % 10
            
            # 重力適用
            final_idx = apply_gravity(target_idx, role)
...
if 'analyzed' not in st.session_state:
    # N4 取得 & 分析
    n4_last, n4_trends = fetch_history('N4')
    st.session_state.n4_res = n4_last
    st.session_state.n4_preds = generate_best_mix('N4', n4_last, n4_trends)
    
    # N3 取得 & 分析
    n3_last, n3_trends = fetch_history('N3')
    st.session_state.n3_res = n3_last
    st.session_state.n3_preds = generate_best_mix('N3', n3_last, n3_trends)
...
for g in ['L7', 'L6', 'ML', 'B5', 'N4', 'N3', 'NM', 'KC']:
    if g == 'N4':
        data_map[g] = st.session_state.n4_preds
    elif g == 'N3':
        data_map[g] = st.session_state.n3_preds
    elif g == 'NM':
        # MiniはN3の下2桁を抽出
        data_map[g] = [p[-2:] for p in st.session_state.n3_preds]
    else:
        data_map[g] = ["COMING SOON"] * 10

# 表示用ラストリザルト切り替え
def get_display_last(g):
    if g == 'N4': return st.session_state.n4_res
    if g == 'N3': return st.session_state.n3_res
    if g == 'NM': return st.session_state.n3_res[-2:] # Miniの前回値
    return "----"

current_last_val = get_display_last(st.session_state.get('g', 'N4'))

# --- UI構築 (MIRU-PAD Layout) ---
html_code = f"""
<!DOCTYPE html>
<html>
<head>
...
<body>
    <div class="lcd">
        <div id="lcd-game-label" class="lcd-label">LAST RESULT ({st.session_state.g}): {current_last_val}</div>
    ...
    <script>
        ...
        // Python側で取得したLAST結果をマッピング
        const lastResults = {{
            'N4': '{st.session_state.n4_res}',
            'N3': '{st.session_state.n3_res}',
            'NM': '{st.session_state.n3_res[-2:]}',
            'L7': '----', 'L6': '----', 'ML': '----', 'B5': '----', 'KC': '----'
        }};
        
        let currentGame = '{st.session_state.g}';
        let currentCount = {st.session_state.c};

        function updateDisplay() {{
            const box = document.getElementById('preds-box');
            const countLabel = document.getElementById('count-display');
            const label = document.getElementById('lcd-game-label');
            
            countLabel.innerText = currentCount + ' 口';
            label.innerText = 'LAST RESULT (' + currentGame + '): ' + lastResults[currentGame];

...
def generate_predictions(game_type, last_val, trends):
    ...
    preds = []
    seen_tails = set() # 被りチェック用（ミニ）

    for role in roles:
        # 被りが出た場合のみ、ドラムを再調整するループ
        for attempt in range(10): # 最大10回スピン調整
            row = ""
            for i, col in enumerate(cols):
                ...
                t_spin = trends[col]
                if attempt > 0:
                    t_spin = (t_spin + random.choice([1, -1, 5, 2, -2])) % 10
...
if 'analyzed' not in st.session_state:
    ...
    st.session_state.n4_p = generate_predictions('N4', n4_l, n4_t)
    st.session_state.n3_res = n3_l
    st.session_state.n3_p = generate_predictions('N3', n3_l, n3_t)
...
d_map = {
    ...
    'NM': [p[-2:] for p in st.session_state.n3_p],

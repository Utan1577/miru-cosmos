...
# =========================
# MIRU-PAD (RAKUTEN N4/N3 + MONEYPLAN KC / AUTO)
# =========================
...
def ensure_predictions_for_round(game: str, round_no: int, base_last: str, base_trends: dict, base_pred_func) -> list[str]:
    ...
    if game == "KC":
        # KCは履歴トレンドを使わずランダムシードで生成
        preds = base_pred_func()
    else:
...
def build_pages_for_game(game: str, items: list, months_used: list[int]) -> dict:
    digits = 4 if game in ["N4", "KC"] else 3
    ...
    for it in (items or []):
        ...
        if game != "KC":
            ...
            # KCは絵文字4つ (サロゲートペア対応はlenで簡易チェック、厳密にはしない)
            if len(d.get("num", "")) < 4: 
                continue
    ...
    if game == "KC":
        now_preds = ensure_predictions_for_round(game, next_round, "", {}, pred_func_kc)
    else:
    ...
    for it in norm_items:
        ...
        if game == "KC":
            preds = ensure_predictions_for_round(game, rno, "", {}, pred_func_kc)
        else:
            preds = ensure_predictions_for_round(game, rno, seed_last, trends, pred_func_wrapper)
  ...
  <script>
    ...
    function renderResultPanel(page){{
      ...
      if(res) {{
        ...
        const cls = (curG==='KC') ? "result-win kc-emoji" : "result-win";
    ...
    function renderPredPanel(page){{
      ...
      for(let i=0;i<Math.min(curC,preds.length);i++) {{
        const v=preds[i];
        const cls = (curG==='KC') ? "num-text kc-emoji" : "num-text";

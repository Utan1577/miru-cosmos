...
# --- データ取得 (より強力な偽装とリトライ) ---
def get_n4_result_strong():
    # ブラウザからのアクセスに見せかける魔法のヘッダー
    headers = {
    ...
    for url in urls:
        try:
            ...
            # 抽せん数字が太字で入っているstrongタグを探す
            strong_tag = soup.select_one('td.alnCenter strong')
            if strong_tag:
                return strong_tag.text.strip()
            # もし上記でダメなら別のパターン
            alt_tag = soup.find('td', class_='alnCenter')
            if alt_tag:
                return alt_tag.text.strip().replace(' ', '')
        except:
            continue
    return "非同期"
...
if 'last_val' not in st.session_state:
    st.session_state.last_val = get_n4_result_strong()

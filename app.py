...
from core.model import (
    ...
    load_pred_store,
    save_pred_store,
    ensure_predictions_for_round_store,
)
...
# NM: result & payout come from N3 (confirmed by you)
    ...
    <script>
        ...
        function renderResultPanel(page) {{
            ...
            if(curG === 'L7' || curG === 'L6' || curG === 'ML' || curG === 'B5') {{
                return `<div class="result-line">[RESULT]</div><div class="result-line">COMING SOON</div>`;
            }}
            ...
            if(res) {{
                ...
                h += `<div class="result-line">当選:</div>`;
            }}
            h += `<div class="result-spacer"></div>`;

            if(curG === 'NM') {{
                // Mini payout is on Numbers3 page (STR)
                const miniYen = payoutYen(pay, "STR");
                if(miniYen) h += `<div class="result-line">Mini: ${{escHtml(miniYen)}}</div>`;
                else h += `<div class="result-line">Mini:</div>`;
                return h;
            }}
        ...
        // STR=red (position match), BOX=blue (digit match remaining)

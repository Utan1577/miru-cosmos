...
# ------------------------------------------------------------
# UI state (miru_status.json) - keep non-empty so safe_save_json can write
# ------------------------------------------------------------
def load_ui_state():
...
def _ensure_game(store: dict, game: str):
    g = store["games"].get(game)
    if not g or "preds_by_round" not in g:
        store["games"][game] = {"preds_by_round": {}, "history_limit": 120}
    return store["games"][game]

def ensure_preds(game: str, round_no: int, builder):
    g = _ensure_game(pred_store, game)
    preds_by_round = g["preds_by_round"]
    key = str(round_no)
...
# ------------------------------------------------------------
# Numbers pages build
# - per-page trends (slice history) to avoid same digits
# - payouts come from core.fetch (already normalized keys for frontend)
# ------------------------------------------------------------
def build_numbers_pages(game: str, items: list[dict]):
    ...
    def now_builder():
        ...
        raw = generate_predictions(game, latest["num"], trends)
        return distill_predictions(game, raw, out_n=10)

    now_preds = ensure_preds(game, next_round, now_builder)
    ...
    for i, it in enumerate(items):
        sub = items[i:]  # history as of that time
        history_nums = [[int(c) for c in x["num"]] for x in sub if str(x.get("num", "")).isdigit()]
        trends = calc_trends_from_history(history_nums, cols)
        prev = sub[1] if len(sub) > 1 else None
        seed_last = prev["num"] if prev else it["num"]
        rno = int(it.get("round", 0))

        def builder(seed=seed_last, tr=trends):
            raw = generate_predictions(game, seed, tr)
            return distill_predictions(game, raw, out_n=10)

        preds = ensure_preds(game, rno, builder)
...
# ------------------------------------------------------------
# KC results (money-plan only) build by date mapping
# ------------------------------------------------------------
...
def moneyplan_latest_round() -> int | None:
    try:
        r = requests.get(MP_BASE, headers=HEADERS, timeout=15)
        r.encoding = r.apparent_encoding
        html = r.text
        rounds = [int(x) for x in re.findall(r"/round/(\d+)/", html)]
        if rounds:
            return max(rounds)
    except Exception:
        return None
...
def moneyplan_fetch_round(round_no: int):
    ...
    return {"kc_round": round_no, "date": date, "result": result, "payout": payout}
...
def moneyplan_build_date_map(target_dates: set[str], max_scan: int = 400):
    ...
    while rno >= 1 and scanned < max_scan and len(out) < len(target_dates):
        ...
        try:
            ...
            if item and item["date"] and item["date"] in target_dates and item["date"] not in out:
                out[item["date"]] = item
        except Exception:
            pass
        rno -= 1
...
# Fetch results
# ------------------------------------------------------------
n4_items, _ = fetch_last_n_results("N4", need=30)
n3_items, _ = fetch_last_n_results("N3", need=30)
...
# KC pages synced to N4 round/date; results from money-plan by date
target_dates = set()
for p in n4_pages:
    if p["mode"] == "RESULT":
...
# write non-empty UI state once (keeps miru_status.json from staying empty forever)
try:

...
# =========================
# MIRU-PAD (RAKUTEN N4/N3 + TAKARAKUJI-NET KC / AUTO)
# =========================
...
def fetch_kc_results_robust(need: int = 20):
    """
    着せかえクーちゃんの過去データを「宝くじネット」から取得する。
    一覧性が高く、構造が安定しているため採用。
    """
    url = "https://www.takarakujinet.co.jp/kisekae/"
    
    try:
        # 宝くじネットはShift_JISの場合が多いが、requestsが自動判定する
        ...
        # 多くのサイトで使われる一般的なテーブル構造を探す
        # "第" と "回" を含む行を探してデータ化する汎用ロジック
        ...
        for tr in rows:
            ...
            # 日付抽出 (yyyy年mm月dd日 or yyyy/mm/dd)
            m_date = re.search(r"(\d{4}[年/]\d{1,2}[月/]\d{1,2}日?)", text)
            date_str = m_date.group(1) if m_date else ""

            # 絵柄の抽出 (imgタグのalt または テキストから判定)
            # このサイトは画像(alt=リンゴ)を使っていることが多い
            ...
            # 画像で取れなかった場合、テキスト内のカタカナを探す
            if len(fruits) != 4:
                fruits = [] # リセット
                # 行内のテキストを全走査
                row_text = tr.get_text(" ", strip=True)
                # 単純にテキスト出現順で拾う（誤爆の可能性はあるがクーちゃんページなら低い）
                # 正規表現でフルーツ名を探す
            ...
            # 払戻金
            # 行内のテキストから金額抽出 "1等 x,xxx円"
            payout = {}
            row_text_clean = tr.get_text(" ", strip=True)
            
            # 金額抽出ロジック（宝くじネットの表記に合わせる）
            # 例: 1等 6,000円 2等 900円 3等 100円
        ...
        # 結果がない場合、もう一つの有力サイト「宝くじロトハウス」も試す（チェーン）
        if not items:
...
def fetch_kc_results_backup(need: int = 20):
    ...
    try:
        ...
        for tr in rows:
            ...
            for img in imgs:
                src = img.get("src", "")
                # srcファイル名判定 (apple.gif, mikan.gif etc)
                if "ringo" in src or "apple" in src: fruits.append("🍎")
                elif "mikan" in src or "orange" in src: fruits.append("🍊")
                elif "melon" in src: fruits.append("🍈")
                elif "budou" in src or "grape" in src: fruits.append("🍇")
                elif "momo" in src or "peach" in src: fruits.append("🍑")
            ...
            items.append({
                "round": round_no,
                "date": "", # 日付取りにくい場合は空でもOK
                "num": "".join(fruits),
                "payout": payout
            })
            if len(items) >= need: break
        return items, []
    except:
        return [], []
...
<head>
  ...
  <style>
    ...
    /* KC用フォント調整: サイズを落とし、詰め込みすぎないようにして崩れを防ぐ */
    .kc-font {{
      font-family: "Apple Color Emoji", "Segoe UI Emoji", "Noto Color Emoji", sans-serif;
      letter-spacing: 0.05em !important; /* 少し間隔を空ける */
      font-size: 15px !important;      /* 小さくしすぎず、ちょうど良いサイズに */
    }}

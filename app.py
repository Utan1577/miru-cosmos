...
def generate_predictions(game_type, last_val, trends):
    ...
    for role in roles:
        ...
        for i, col in enumerate(cols):
            ...
            elif role == 'shift': spin = (trends[col] + random.choice([1, -1, 5])) % 10
            else: spin = trends[col] if random.random() > 0.2 else (trends[col] + 1) % 10

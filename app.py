...
# HTML/JavaScript 一体型コード
# ボタンを押しても「?g=...」とかのURL移動をせず、JSだけで画面を書き換える
html_code = f"""
<!DOCTYPE html>
<html>
<head>
    ...
    <style>
        ...
        .lcd-label {{ font-size: 11px; color: #444; font-weight: bold; margin-bottom: 2px; }}
        /* 数字：口数に合わせてサイズが自動で縮小される魔法 */
        .preds-container {{ display: flex; flex-direction: column; justify-content: center; align-items: center; transition: all 0.2s; }}
        .num-text {{ font-family: 'Courier New', monospace; font-weight: bold; letter-spacing: 5px; line-height: 1.1; }}
...
<body>
    <div class="lcd">
        ...
        <div style="width:70%; border-top:1.5px solid #777; margin:5px 0;"></div>
    ...
    <div class="pad-grid">
        ...
        <a class="btn btn-loto" onclick="setGame('L6')">LOTO 6</a>
    ...
    <script>
        ...
        function updateDisplay() {{
            ...
            // 数字のサイズ計算 (口数が多いほど小さくする)
            let fontSize = 36;
            if (currentCount > 3) fontSize = 28;
            if (currentCount > 5) fontSize = 22;
            if (currentCount > 8) fontSize = 18;
...
# HTMLを埋め込み (heightを固定して、Streamlitの干渉を断つ)

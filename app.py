...
# --- UI構築（HTML/CSSをcomponents.htmlで描画） ---
game = st.session_state.g
last = get_n4() if game == 'N4' else "----"
preds = gen_p(game, st.session_state.c)
preds_html = "".join([f'<div style="font-size:32px; font-weight:bold; letter-spacing:5px;">{p}</div>' for p in preds])
...
<head>
    ...
    <style>
        body {{ background-color: #000; color: #fff; font-family: sans-serif; margin: 0; padding: 5px; overflow: hidden; }}
        .lcd {{ background-color: #9ea7a6; color: #000; border: 4px solid #555; border-radius: 10px; height: 180px; display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; box-shadow: inset 0 0 10px rgba(0,0,0,0.5); }}
        .count-bar {{ display: flex; justify-content: space-between; align-items: center; background: #222; padding: 10px 20px; border-radius: 30px; margin: 10px 0; }}
        .btn-round {{ width: 45px; height: 45px; border-radius: 50%; background: #444; color: white; text-decoration: none; display: flex; justify-content: center; align-items: center; font-size: 24px; font-weight: bold; border: 1px solid #666; }}
        .pad-grid {{ display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }}
        .btn {{ height: 50px; border-radius: 20px; color: white; font-weight: bold; font-size: 13px; display: flex; justify-content: center; align-items: center; text-decoration: none; border: 2px solid rgba(0,0,0,0.2); box-shadow: 0 4px #000; }}
...
<body>
    <div class="lcd">
        <div style="font-size:10px; color:#444;">LAST RESULT ({game}): {last}</div>
        <div style="width:60%; border-top:1px solid #777; margin:5px 0;"></div>
        {preds_html}
    </div>

    <div class="count-bar">
        <a href="?a=m&g={game}" target="_self" class="btn-round">－</a>
        <div style="font-size:18px; font-weight:bold;">{st.session_state.c} 口</div>

...
def parse_month_page(month_url: str, digits: int) -> list[dict]:
    ...
    # Month page is "inline text", so scan by text lines.
    ...
    while i < len(lines):
        if lines[i] == "開催回" and i + 1 < len(lines):
            ...
            dtxt = None
            ntxt = None
            payout = {}  # STR/BOX/SET-S/SET-B/MINI

            # --- Scan THIS block only: from this "開催回" until the next "開催回"
            end = len(lines)
            ...
            for j in range(i, end):
                ...
                # Numbers3 mini:
                # Do NOT rely on j+1/j+2 being perfectly aligned.
                # Search nearby for "xx口" and "x円".
                if digits == 3 and ("ミニ" in lines[j]):
                    kuchi = ""
                    yen = ""
                    for k in range(j + 1, min(j + 12, end)):
            ...
            # move to next block (safe / faster)
            i = end
            continue

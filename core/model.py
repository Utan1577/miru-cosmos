...
def distill_predictions(game: str, raw_preds: list[str], out_n: int = 10) -> list[str]:
    if not raw_preds:
        return ["0" * (4 if game == "N4" else 3)] * out_n
    ...
    if digits is None:
        ...
        out = shuffle_recompose(game, out) if game in ("N4", "N3") else out
        return out

    # simple: keep unique first, then pad deterministically
    outs = []
    seen = set()
    for s in raw_preds:
        s = str(s)
    ...
    base = outs[-1] if outs else ("0" * digits)
    k = 0
    while len(outs) < out_n and k < 5000:
        ...
        cand = "".join(cc)
        if cand not in seen and len(cand) == digits:
            seen.add(cand)
            outs.append(cand)
    ...
    outs = shuffle_recompose(game, outs)  # ★最終出口で必ずシャッフル

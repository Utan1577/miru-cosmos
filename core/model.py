...
# ------------------------------------------------------------
# gravity (config互換：dict/list 両対応)
# ------------------------------------------------------------
def _get_sectors(obj, col: str):
    # obj can be dict (per-col) OR list/tuple/set (global sectors)
...
def apply_gravity_final(col: str, idx: int, role: str) -> int:
    ...
    if role == "ace":
        ...
        if sectors:
            if idx in sectors:
                return idx
            # 70% chance to pull toward sector
...
def distill_predictions(game: str, raw_preds: list[str], out_n: int = 10) -> list[str]:
    """二段蒸留（濃縮）: 原液10本を素材に、共起（同居）ペア核で再編成して10本に収束させる。"""
    if not raw_preds:
        return []

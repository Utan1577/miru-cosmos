...
def kc_random_10() -> list[str]:
    ...
    # 1. 仮のシード値（前回数字）とトレンドをランダム生成
    # (KC専用の履歴追跡はしていないため、起動ごとにシードを変えてN4エンジンを回す)
    ...
    for num_str in raw_nums:
        ...
        for digit in num_str:
            row += KC_FRUIT_MAP.get(digit, "🍎") # 万が一のError時はリンゴ
